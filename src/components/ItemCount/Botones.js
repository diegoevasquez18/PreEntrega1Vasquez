...
const Botones = ({ stock = 0, initial = 1, handleOnAdd }) => {
  ...
  return (
    <div className="botonesItems">
      <BotonAgregar texto="-" botonDeClick={false} manejarClick={retirar} />
      <Contador onAdd={numContador} />
      <BotonAgregar texto="+" botonDeClick={true} manejarClick={agregar} />
      <div className="comprar">
        <Button variant="success" onClick={() => handleOnAdd(numContador)}>
          Comprar
        </Button>
      </div>
    </div>
